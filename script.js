    ...
    <div class="pdf-page" style="width: 816px; height: 1056px; background: white; position: relative; font-family: -apple-system, sans-serif;">
        <div style="padding: 50px 40px;">
            <div style="background: #1a3a2e; color: white; padding: 15px 20px; margin: -50px -40px 30px -40px;">
                <h2 style="margin: 0; font-size: 24px; font-weight: 700;">Summary of Analysis</h2>
            </div>
            <p style="color: #1a3a2e; font-size: 14px; margin: 20px 0;">This report provides a comparison of AI models based on user-defined queries. Below are the results of each model's performance along different metrics.</p>
...
for (var j = 0; j < results.length; j++) {
    var modelResult = results[j];
    var specScore = modelResult.scores.split('Specificity: ')[1].split('\n')[0];
    var actScore = modelResult.scores.split('Actionability: ')[1].split('\n')[0];
    var depthScore = modelResult.scores.split('Domain Depth: ')[1].split('\n')[0];
    pagesHTML += `
                    <tr style="background: ${j % 2 === 0 ? '#f7f4ea' : 'white'};">
                        <td style="padding: 10px; border-bottom: 1px solid #e0e0e0; font-weight: 600; color: #1a3a2e;">${escapeHtml(modelResult.shortId || modelResult.model)}</td>
                        <td style="padding: 10px; border-bottom: 1px solid #e0e0e0; text-align: center; color: #2a2a2a;">${specScore}/10</td>
                        <td style="padding: 10px; border-bottom: 1px solid #e0e0e0; text-align: center; color: #2a2a2a;">${actScore}/10</td>
                        <td style="padding: 10px; border-bottom: 1px solid #e0e0e0; text-align: center; color: #2a2a2a;">${depthScore}/10</td>
                    </tr>`;
} 
